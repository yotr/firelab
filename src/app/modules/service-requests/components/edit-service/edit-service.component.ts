...
export class EditServiceComponent implements OnInit, AfterViewInit {
  ...
  // get current data
  getCurrentData() {
    this.apiService.getById('services', this.updateId).subscribe({
      next: (data: any) => {
        //  set values
        if (data.isSuccess) {
          console.log(data);
  ...
  submit() {
    if (this.addForm.valid) {
      ...
      this.uploading = true;
      // api
      this.apiService.update('services', this.updateId, data).subscribe({
        next: (data) => {
          console.log(data);
          if (data?.isSuccess) {
            if (this.currentLanguage == 'ar') {
            ...
            this.router.navigate(['/modules/serviceRequests']);
          }
        },
        error: (err: any) => {
          if (this.currentLanguage == 'ar') {
