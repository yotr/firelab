...
export class PermissionsComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.dataKeys = [
      {
        name: 'name',
        display: this.translateService.instant('permissions.table.name'),
        type: 'string',
        active: true,
      },
      // {
  ...
  search(event: any) {
    if (event?.value != null && event.value?.trim() != '') {
      this.apiService
        .globalSearch('roles/globalsearch', event?.value, event?.column)
        .subscribe({
          next: (data: any) => {
            if (data?.isSuccess) {
              console.log(data);
  ...
  delete(deleteId: any) {
    ...
    this.apiService.delete('roles', deleteId).subscribe({
      next: (data) => {
        if (data?.isSuccess) {
