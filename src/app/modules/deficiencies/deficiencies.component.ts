...
export class DeficienciesComponent implements OnInit, AfterViewInit {
  // current language
  ...
  constructor(
    ...
    this.dataKeys = [
      {
        name: 'name',
        display: this.translateService.instant('deficiencies.table.name'),
        type: 'string',
        active: true,
      },
    ];
    ...
    this.apiService
      ...
      .subscribe({
        next: (data: any) => {
          console.log(data);
          if (data?.isSuccess) {
            this.data = data?.value?.deficiencies;
  ...
  search(event: any) {
    if (event?.value != null && event.value?.trim() != '') {
      this.loading = true;
      this.apiService
        .globalSearch('deficiencies/globalsearch', event?.value, event?.column)
        .subscribe({
          next: (data: any) => {
            if (data?.isSuccess) {
              console.log(data);
  ...
  delete(deleteId: any) {
    ...
    this.apiService.delete('deficiencies', deleteId).subscribe({
      next: (data) => {
        console.log(data);
        if (data?.isSuccess) {
