...
@Component({
  selector: 'app-warranty-contract-module-table',
  templateUrl: './warranty-contract-module-table.component.html',
  styleUrls: ['./warranty-contract-module-table.component.css'],
})
export class WarrantyContractModuleTableComponent implements OnInit {
 //variables
  @Input() data: any[] = [];
  @Input() dataKeys: any[] = [];
  ...
  deleteItems() {
    ...
    if (this.deletedData.length > 0) {
      // api
      this.apiService
        .deleteMulti(`warrantyContract/delete-multi`, this.deletedData)
        .subscribe({
          next: (data) => {
            console.log(data);
            if (data?.isSuccess) {
