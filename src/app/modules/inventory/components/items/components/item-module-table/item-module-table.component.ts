...
@Component({
  selector: 'app-item-module-table',
  templateUrl: './item-module-table.component.html',
  styleUrls: ['./item-module-table.component.css'],
})
export class ItemModuleTableComponent implements OnInit {
  //variables
  @Input() data: any[] = [];
  @Input() dataKeys: any[] = [];
  ...
  deleteItems() {
    ...
    if (this.deletedData.length > 0) {
      // api
      this.apiService
        .deleteMulti(`items/delete-multi`, this.deletedData)
        .subscribe({
          next: (data) => {
            console.log(data);
            if (data?.isSuccess) {
