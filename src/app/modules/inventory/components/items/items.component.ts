...
export class ItemsComponent implements OnInit {
  ...
  constructor(
    ...
    this.dataKeys = [
      {
        name: 'name',
        display: this.translateService.instant('items.table.name'),
        type: 'string',
        active: true,
      },
      {
        name: 'cost',
        display: this.translateService.instant('items.table.cost'),
        type: 'string',
        active: true,
      },
      {
        name: 'quantity',
        display: this.translateService.instant('items.table.quantity'),
        type: 'string',
        active: true,
      },
    ];
  ...
  search(event: any) {
    if (event?.value != null && event.value?.trim() != '') {
      this.loading = true;
      this.apiService
        .globalSearch('Items/globalsearch', event?.value, event?.column)
        .subscribe({
          next: (data: any) => {
            if (data?.isSuccess) {
              console.log(data);
  ...
  delete(deleteId: any) {
    ...
    this.apiService.delete('Items', deleteId).subscribe({
      next: (data) => {
        console.log(data);
        if (data?.isSuccess) {
